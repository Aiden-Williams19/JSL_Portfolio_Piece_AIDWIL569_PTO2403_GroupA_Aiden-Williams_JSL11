...
function fetchAndDisplayBoardsAndTasks() {
  ...
  if (boards.length > 0) {
    ...
    activeBoard = localStorageBoard ? localStorageBoard : boards[0];  // Correct ternary operator
...
function displayBoards(boards) {
  ...
  boards.forEach(board => {
    ...
    boardElement.addEventListener('click', () => {  // Fixed the click event
...
function filterAndDisplayTasksByBoard(boardName) {
  ...
  elements.columnDivs.forEach(column => {
    ...
    filteredTasks.filter(task => task.status === status).forEach(task => {  // Fixed comparison operator
...
function addTaskToUI(task) {
  const column = document.querySelector(`.column-div[data-status="${task.status}"]`);  // Correct template literal
  if (!column) {
